...
EnvelopeEditor::EnvelopeEditor(
    const String& prefix) : OpenGlLineRenderer(kTotalPoints),
...
bitklavier::poly_float EnvelopeEditor::getBackupPhase(bitklavier::poly_float phase) {
  bitklavier::poly_float backup = 0.0f;
...
float EnvelopeEditor::getEnvelopeValue(float t, float power, float start, float end) {
  return start + (end - start) * bitklavier::futils::powerScale(t, power);
}
...
inline bitklavier::poly_float EnvelopeEditor::getOutputsTotal(
...
void EnvelopeEditor::render(OpenGlWrapper& open_gl, bool animate) {
  ...
  bitklavier::poly_float phase = (input_phase - bitklavier::kVoiceOn) * (1.0f / phase_length);
  phase = bitklavier::utils::maskLoad(phase, 1.0f, off_mask);
  phase = bitklavier::utils::min(phase, 1.0f);

  bitklavier::poly_mask reset_mask = bitklavier::poly_float::greaterThan(last_phase_, phase);
  bitklavier::poly_float backup_phase = getBackupPhase(phase);
  last_phase_ = bitklavier::utils::maskLoad(last_phase_, backup_phase, reset_mask);
